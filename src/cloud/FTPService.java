...
public class FTPService {
	...
	public FTPService authorize(String USERNAME, String PASSWORD) throws Exception {
	...
	public void download(String local_fullpath, String server_fullpath) throws Exception {
		...
		if (!temp.exists()) throw new Exception("[FTPService] Download failed");
	}

	public void downloadArray(String local_path, String[] server_fullpath) throws Exception {
		for (String fullpath : server_fullpath) {
			File temp = new File(local_path + fullpath.substring(fullpath.lastIndexOf("/"), fullpath.length()));
			ftp.download(fullpath, temp);
			if (!temp.exists()) throw new Exception("[FTPService] Download failed");
		}
	}

	public void upload(String local_fullpath, String server_path) throws Exception {
		File temp = new File(local_fullpath);
		ftp.changeDirectory(server_path);
		ftp.upload(temp);
	}

	public void uploadArray(String[] local_fullpath, String server_path) throws Exception {
		ftp.changeDirectory(server_path);
